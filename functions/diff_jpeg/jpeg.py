...
def diff_jpeg_coding(
    ...
    # Get original shape
    _, _, H, W = image_rgb.shape  # type: ignore # type: int, int, int, int
    # Perform encoding
    # [-1, 1] to [0, 255]
    image_rgb = (image_rgb + 1) / 2 * 255
    ...
    )  # type: ignore # type: Tensor, Tensor, Tensor
    ...
    # [0, 255] to [-1, 1]
    image_rgb_jpeg = image_rgb_jpeg / 255 * 2 - 1
...
class DiffJPEGCoding(nn.Module):
    ...
    def __init__(self, jpeg_qf, device, ste: bool = False) -> None:
        """Constructor method.
        ...
        # Save parameter
        self.ste: bool = ste
        self.jpeg_qf = torch.tensor([float(jpeg_qf)]).to(device)
        self.device = device
    ...
    def A(self, data, **kwargs):
        return self.forward(data, self.jpeg_qf)