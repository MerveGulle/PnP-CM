...
class JPEG_ArtifactRemoval():
    def __init__(self, jpeg_qf, device):
        self.device = device
        self.jpeg_qf = jpeg_qf  
        
        self._jpeg_encode = partial(jpeg_encode, qf=self.jpeg_qf)
        self._jpeg_decode = partial(jpeg_decode, qf=self.jpeg_qf)
        
    def A(self, data, **kwargs):
        
        jpeg_en = self._jpeg_encode(data)
        output = self._jpeg_decode(jpeg_en)
