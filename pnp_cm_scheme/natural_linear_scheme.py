...
def pnp_cm_natural_linear_restoration(
        ATy,
        x_init,
        model,
        A_funcs,
    ...
    xt = x_init.clone()
    xt_moment = x_init.clone()
    ut = torch.zeros_like(x_init)
    ut_moment = torch.zeros_like(x_init)
    ...
    for at in tqdm(zip(alphas)):
        ...
        zt = A_funcs.AtA_add_eta_inv(ATy + penalty_param[iter_ind] * (xt_moment-ut_moment), eta=penalty_param[iter_ind]).reshape(*ATy.size())        
